...
var videoSpeedElement = document.getElementById('speed-controls');
/* Video interface */
...
var speedIndex = 1, currentRate;

videoSpeedElement.addEventListener('click',  function(){
  if (!video()) {
    return;
  }
  var rates = [ 1, 1.5, 2];
  currentRate = rates[speedIndex];
  speedIndex = ++speedIndex%rates.length;


  updateSpeed(currentRate);
  changeVideoSpeed(currentRate);
...
function updateSpeed(rate) {
  document.querySelector(".speed-indicator").innerHTML = rate + "x";
  durationIndicatorElement.innerHTML = video() ? formatTime(video().duration/rate) : '-';
...
var vid;
  multiResVideo.element().playbackRate = rate;
